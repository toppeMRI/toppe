...
%
% Outputs:
%  TOPPE files are written to disk with name 'tarfile'
%
% Usage examples:
...
%
% $Id: tmp.m,v 1.1 2018/10/23 15:36:18 jfnielse Exp $
% $Source: /export/home/jfnielse/Private/cvs/projects/psd/toppe/matlab/pulseq/tmp.m,v $
...
for ib = 2:length(seq.blockEvents)
	...
	if modCandidate.hasADC
		...
		sl   = mod(floor(adcCount/arg.system.maxView) + 1, arg.system.maxSlice);   
		if sl == 0
			sl = arg.system.maxSlice;
		end
	...
	for ic = 1:length(moduleArr)
		if (moduleArr(ic).nt == modCandidate.nt ...
			...
			isUnique = 0;
			break;   % break out of 'for ic' loop. 'ic' now has the value of a module we'll add to/reuse
		end
	end
   ...
   % so now check to see if we can find an existing set of waveforms (a column in moduleArr(ic).rf/gx/gy/gz)
	...
	if canReuse
		...
		loopStructArr(ib) = updateLoopStruct([], block, nextblock, arg.system, ...
			'dabmode', 1, 'slice', sl, 'echo', echo, 'view', view, 'mod', ic, 'wavnum', iwav);
	end
...
fprintf(fid,'Total number of unique moduleArr\n');
fprintf(fid,'%d\n', length(moduleArr));
...
if isempty(arg)
	...
	arg.dabmode = 0;         % turn on/off data acquisition (1/0)
