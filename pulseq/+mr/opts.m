...
if isempty(parser)
    ...
    parser.addParamValue('rfDeadTime',100e-6,@isnumeric);
    parser.addParamValue('rfRingdownTime',30e-6,@isnumeric);
    parser.addParamValue('adcDeadTime',20e-6,@isnumeric);
...
if isempty(opt.maxGrad)
    maxGrad = mr.convert(32,'mT/m');    % Default: 32 mT/m
else
    maxGrad = mr.convert(opt.maxGrad,opt.gradUnit,'Hz/m');
end
if isempty(opt.maxSlew)
    maxSlew=mr.convert(130,'T/m/s');	% Default: 130 mT/m/ms
else
