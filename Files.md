...
(column 16; to set that column, pass the 'waveform' argument to toppe.write2loop()).
...
i.e., it is _not_ necessary to explicitly store each gradient rotation in the `.mod` file 
(in fact, it's a rather bad idea as it consumes memory unnecessarily).
